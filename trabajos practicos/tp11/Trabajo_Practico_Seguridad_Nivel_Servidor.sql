...
-- c) Luego de cambiar la autenticacion, debo reiniciar el SQL Server para que se apliquen los cambios
...
-- b) 
alter login [CarlosRodriguez] with password = 'NuevaVentas2026!';

alter login [LauraMartinez] with DEFAULT_database = [ProyectoX];
...
-- b)
alter server role [dbcreator] add member [AnaGarcia];

-- c)
alter server role [bulkadmin] add member [CarlosRodriguez];
